    ...
    try {
        stopLists[targetRoute].forEach(stopId => {
            ...
            if (lowestBusName) {
                ...
                setStopEtaLabel(stopId, lowestETAMin + (overtime ? '+ min' : ' min'), true);
            } else {
...
function syncPostHogPersonProfile() {
    ...
    const personProps = {
        ...
        chatbot_model: currentSettings['chatbot-model'] || defaultSettings['chatbot-model'],
...
function flyToStop(stopId, fromUserInteraction = false) {
    ...
    flyToCenteredBelow([loc.lat, loc.long], 16, document.querySelector('.stop-info-popup .stop-info-popup-inner'), 0.5);
